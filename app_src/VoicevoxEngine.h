...
public:
    ...
    void SetSpatialPosition(int centerX, int screenWidth, int emphasis);
...
private:
    struct AudioItem {
        ...
        double leftDelaySeconds = 0.0;
        double rightDelaySeconds = 0.0;
        double leftGain = 1.0;
    ...
    struct TextItem {
        ...
        uint32_t speakerId;
        double leftDelaySeconds;
        double rightDelaySeconds;
        double leftGain;
        double rightGain;
        double volume;
    };
    ...
    std::atomic<double> outputVolume_{1.0};
    std::atomic<uint32_t> speakerId_{23};
    std::atomic<double> spatialLeftDelaySeconds_{0.0};
    std::atomic<double> spatialRightDelaySeconds_{0.0};
